...
bool String::operator==(const String& rhs)
{
    if (this->size() != rhs.len) //compare the size
    {
        return false; //automatically return false if the sizes are not the same
    }
        //check to see if the data of the Strings are equal
        for (int i = 0; i < this-> size(); i ++)
        {
        ...
        return true; //return true if both the size and the characters are the same
}
*/
...
bool String::operator> (const String& rhs)
{
    ...
    {
        //while values of both strinngs are the same
        while (data[index] == rhs.data[index])
        {
            //increment index
            index++;
        }
        ...
        if (data[index] != rhs.data[index])
        {
            ...
            //if right is bigger
            else if (rhs.data[index] > data[index])
            {
                return false;
            }
        }
...
/*
    //start for-loop with upperbounds being the starting poition of string rhs since the left side isnt being modified, just concatenated
    for (int i = this->size(); i < temp.len; i ++)
    {
        temp.data[i] = rhs.data[tempIndex];
